...
- **Closed-set classification:** The model is trained on 6 fruit classes only. Since it uses a softmax output, it will always assign a probability distribution across these 12 known classes — even for out-of-distribution inputs (e.g., a non-fruit image, or a fruit outside the trained classes). This can occasionally result in high-confidence predictions on unsupported inputs.
- **Planned fix:** Retrain with an explicit **"Other/Unknown"** negative class using out-of-distribution images (non-fruit objects and unsupported fruit types), so the model can correctly reject inputs outside its trained scope rather than forcing a prediction.
